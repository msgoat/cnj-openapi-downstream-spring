...
public class WebSecurityConfiguration {
    ...
    SecurityFilterChain filterChain(HttpSecurity http, JwtAuthenticationConverter customConverter) throws Exception {
        http
                .authorizeHttpRequests(authz -> /*authz.anyRequest().permitAll()*/ authz.requestMatchers("/actuator/**", "/openapi*", "/v3/api-docs*", "/swagger-ui/**").permitAll().anyRequest().authenticated())
